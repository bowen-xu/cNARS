...
// TEST(test_parser, test_parse_line12)
// {
//     GTEST_ASSERT_EQ(test_parse_line("(|, {A}, {B}, [A, B],(|, [B, C], {B, C}))."), 0);
//     // GTEST_ASSERT_EQ(test_parse_line("{{A, B}, {C}}."), 0);
// }

TEST(test_parser, test_efficiency1)
{
    for (auto i = 0; i < 1e4; i++)
        TASK::pTask task = NARSESEPARSER::parser->parse_task("<robin-->brid>.");
}

TEST(test_parser, test_efficiency2)
{
    for (auto i = 0; i < 1e4; i++)
        TASK::pTask task = NARSESEPARSER::parser->parse_task("<<(*, $0, room_101) --> enter> =\\> <(*, $0, door_101) --> open>>.");
}

TEST(test_parser, test_efficiency3)
{
    for (auto i = 0; i < 1e4; i++)
        TASK::pTask task = NARSESEPARSER::parser->parse_task("(|, {A}, {B}, [A, B],(|, [B, C], {B, C})).");
}

TEST(test_parser, test_efficiency4)
{
    for (auto i = 0; i < 1e4; i++)
        TASK::pTask task = NARSESEPARSER::parser->parse_task("{{{{{A}, B}, B}, B}, B}.");
}

TEST(test_parser, test_efficiency5)
{
    for (auto i = 0; i < 1e4; i++)
        TASK::pTask task = NARSESEPARSER::parser->parse_task("{A}.");
}

TEST(test_parser, test_efficiency6)
{
    TASK::pTask task = NARSESEPARSER::parser->parse_task("{A}.");
    auto &term = task->term();
    TERM::Term *ptr;
